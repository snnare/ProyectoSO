...
def get_user_image_path():
    root = Tk()
    root.withdraw()  # Para evitar que aparezca la ventana principal de tkinter

    file_path = filedialog.askopenfilename(title="Seleccionar imagen", filetypes=[("Archivos de imagen", "*.jpg;*.png;*.jpeg")])
...
def main():
    try:
        ...
        client.connect(('169.254.94.65', 9999))

        # Solicitar al usuario que elija la imagen
        initial_image_path = get_user_image_path()
