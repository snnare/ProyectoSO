...
def send_and_receive_images(client_socket):
    try:
        while True:
            # Permite al usuario seleccionar una imagen para enviar
            image_path = select_image()

            if not image_path:
                print("No se seleccionó ninguna imagen. Saliendo del envío de imágenes.")
                break
            ...
            with open(rotated_image_path, 'rb') as file:
                ...
                client_socket.send(filename.encode('latin-1'))  # Cambiado a 'latin-1'
                ...
                response_filename = client_socket.recv(response_filename_length).decode('latin-1')  # Cambiado a 'latin-1'
...
if __name__ == "__main__":
    try:
        ...
        client.connect(('192.168.192.174', 5555))

        # Hilo para enviar y recibir imágenes continuamente
        send_and_receive_thread = threading.Thread(target=send_and_receive_images, args=(client,))
